...
map(Routing, Payload) ->
    BindCandidates = get_binding_candidates(Routing),
    
    map_processor(Routing, Payload, BindCandidates).
...
get_binding_candidates(Routing) ->
    case binary:split(Routing, <<".">>, ['global']) of
        [Vsn, Action, Method, Resource | _] ->
            get_binding_candidates(Vsn, Action, Method, Resource);
        [Vsn, Action, Resource| _] ->
            get_binding_candidates(Vsn, Action, Resource);
        [Vsn, Action  | _] ->
...
-spec get_binding_candidates(ne_binary(), ne_binary(), ne_binary()) -> kz_bindings().
get_binding_candidates(Vsn, Action, Resource) ->
    ets:select(table_id(), [{#kz_binding{binding='$1'
                                        ,_='_'
                                        }
                            ,[{'orelse'
                              ,{'=:=', '$1', <<Vsn/binary, ".", Action/binary, ".", Resource/binary>>}
                              ,{'orelse'
                               ,{'=:=', '$1', <<"*.", Action/binary, ".", Resource/binary>>}
                               ,{'orelse'
                                ,{'=:=', '$1', <<Vsn/binary, ".*">>}
                                ,{'=:=', '$1', <<"*.*">>}
                                }
...
-spec get_binding_candidates(ne_binary(), ne_binary(), ne_binary(), ne_binary()) -> kz_bindings().
get_binding_candidates(Vsn, Action, Method, Resource) ->
     ets:select(table_id(), [{#kz_binding{binding='$1'
                                        ,_='_'
                                        }
                            ,[{'orelse'
                              ,{'=:=', '$1', <<Vsn/binary, ".", Action/binary, ".",  Method/binary, ".", Resource/binary>>}
                              ,{'orelse'
                               ,{'=:=', '$1', <<"*.", Action/binary, ".", Method/binary, ".", Resource/binary>>}
                               ,{'orelse'
                                ,{'=:=', '$1', <<Vsn/binary, ".*">>}
                                ,{'=:=', '$1', <<"*.*">>}
                                }
...
fold(Routing, Payload) ->
    %lager:info("kazoo_binding: Routing: ~p ~n",[Routing]),
    BindCandidates = get_binding_candidates(Routing),
    
    fold_processor(Routing, Payload, BindCandidates).
...
init([]) ->
    ...
    ets:new(table_id(), table_options()),
    {'ok', #state{}}.
...
fold_bind_results([#kz_responder{module=M
    ...
    try apply_responder(Responder, Payload) of
        ...
        {'EXIT', {'undef', [{_M, _F, _A, _}|_]}} ->
            ...
            NewPayload = response_error(Payload, Tokens, <<"undef">>),
            fold_bind_results(Responders, NewPayload, Route, RespondersLen, ReRunResponders);
        {'EXIT', _E} ->
            ST = erlang:get_stacktrace(),
            lager:debug("~s:~s/~p died unexpectedly: ~p", [M, F, length(Payload), _E]),
            wh_util:log_stacktrace(ST),
            NewPayload = response_error(Payload, Tokens, <<"died unexpectedly">>),
            fold_bind_results(Responders, NewPayload, Route, RespondersLen, ReRunResponders);
        Pay1 ->
        ...
        'error':'function_clause' ->
            ...
            NewPayload = response_error(Payload, Tokens, <<"function_clause">>),
            fold_bind_results(Responders, NewPayload, Route, RespondersLen, ReRunResponders);
        'error':'undef' ->
            ST = erlang:get_stacktrace(),
            log_undefined(M, F, length(Payload), ST),
        ...
        _T:_E ->
            ST = erlang:get_stacktrace(),
            wh_util:log_stacktrace(ST),
            NewPayload = response_error(Payload, Tokens, <<"Internal Server Error">>),
            fold_bind_results(Responders, NewPayload, Route, RespondersLen, ReRunResponders)
    end;
...
map_processor(_Routing, _Payload, []) ->
    [];
map_processor(Routing, Payload, Bindings) ->
...
response_error([{Req, Pay}|_] = Payload, Tokens, Msg) ->
    case cb_context:is_context(Pay) of 
    true ->  
        Pay1 = cb_context:setters(Pay
...
response_error([Pay | _] = Payload, Tokens, Msg) -> 
    case cb_context:is_context(Pay) of 
    true -> 
        Pay1 = cb_context:setters(Pay
           ,[{fun cb_context:set_resp_status/2, 'error'}
