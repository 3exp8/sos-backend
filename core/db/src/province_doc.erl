...
sumo_wakeup(Doc) ->
	#{
        ...
        default_location =>  maps:get(default_location, Doc, <<"0,0">>),
        status =>  maps:get(status, Doc, <<>>),
        order =>  zt_util:nvl(maps:get(order, Doc, 200),200),
...
sumo_sleep(Info) ->
    ...
    #{
        ...
        default_location =>  maps:get(default_location, Info, <<"0,0">>),
        status =>  maps:get(status, Info, <<>>),
        order =>  maps:get(order, Info, 200),
...
sumo_schema() ->
  sumo:new_schema(?MODULE, [
        ...
        sumo:new_field(default_location, geo_point),
