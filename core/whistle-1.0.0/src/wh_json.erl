...
encode(JObj) -> {Res} = JObj,  jsx:encode(Res).
...
set_value_props(Keys, Value, JObj) when is_list(Keys) -> set_value1_prop(Keys, Value, JObj);
set_value_props(Key, Value, JObj) -> set_value1_prop([Key], Value, JObj).

set_value1_prop([Key| T], Value, JObj) when is_list(JObj) -> 
   JObj ++ [{Key, set_value1_prop(T, Value, [])}];

set_value1_prop([Key| T], Value, ?JSON_WRAPPER(Props)) ->
    Props ++ [{Key, set_value1_prop(T, Value, [])}];

set_value1_prop([], Value, _JObj)  ->  Value. 
