...
-export([pretty_print_bytes/1, 
        pretty_print_bytes/2, 
        unitfy_bytes/2, 
        maybe_unitfy_bytes/2]).
...
put_md(RequestId) -> lager:md([{callid, RequestId}]).
...
rand_hex(Size) ->
    try crypto:strong_rand_bytes(Size) of
        ...
        _:'low_entropy' -> crypto:strong_rand_bytes(Size)
    end.
...
-spec now_s() -> pos_integer().
now_ms() -> now_ms(os:timestamp()).
now_s() -> now_s(os:timestamp()).


s_now(Timestamp) ->
    MegaSecs = Timestamp div 1000000,
    Secs = Timestamp - MegaSecs * 1000000,
    {MegaSecs, Secs, 0}.

us_now(Timestamp) ->
    Mega = Timestamp div 1000000000000,
    SecMicro = Timestamp - (Mega * 1000000000000),
    Sec = SecMicro div 1000000,
    Micro = SecMicro - Sec * 1000000,
    {Mega, Sec, Micro}.
...
unitfy_bytes(Bytes, _Type) when Bytes < ?BYTES_K  ->
    [wh_util:to_binary(Bytes), "B"];
unitfy_bytes(Bytes, Type) when Bytes < ?BYTES_M ->
    K = Bytes div ?BYTES_K,
    [wh_util:to_binary(K), "K", maybe_unitfy_bytes(Bytes rem ?BYTES_K, Type)];
unitfy_bytes(Bytes, Type) when Bytes < ?BYTES_G ->
    M = Bytes div ?BYTES_M,
    [wh_util:to_binary(M), "M", maybe_unitfy_bytes(Bytes rem ?BYTES_M, Type)];
unitfy_bytes(Bytes, Type) when Bytes < ?BYTES_T ->
    G = Bytes div ?BYTES_G,
    [wh_util:to_binary(G), "G", maybe_unitfy_bytes(Bytes rem ?BYTES_G, Type)];
unitfy_bytes(Bytes, Type) ->
    T = Bytes div ?BYTES_T,
    [wh_util:to_binary(T), "T", maybe_unitfy_bytes(Bytes rem ?BYTES_T, Type)].
