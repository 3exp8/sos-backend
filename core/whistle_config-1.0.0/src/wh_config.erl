...
-define(CONFIG_FILE_ENV, "APP_CONFIG").
...
load() ->
    ...
    %     File -> load_file(File)
    % end.
  lager:info("whistle_config: application:get_all_env(whistle_config): ~p",[application:get_all_env(whistle_config)]),
  case  application:get_all_env(whistle_config) of 
    [] ->
      {'ok', ?SECTION_DEFAULTS};
    [{included_applications,_}] -> 
      {'ok', ?SECTION_DEFAULTS}; 
    Props  -> 
        {'ok', lists:keydelete(included_applications, 1, Props)}  
    end.
