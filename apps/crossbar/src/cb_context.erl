...
-export([store/3
         ...
         ,setters/2
         ,new/0
         ,customer_id/1, set_customer_id/2
         ,account_id/1, set_account_id/2
         ,role/1, set_role/2
         ,roles/1, set_roles/2
         ,account_db/1, set_account_db/2
         ,account_doc/1, set_account_doc/2
...
account_doc(#cb_context{db_doc=AcctDoc}) -> AcctDoc.
...
account_realm(Context) ->
    maps:get_value(<<"pvt_realm">>, account_doc(Context), <<>>).
...
context_data(#cb_context{context_data=Data}) -> Data.
...
resp_extra(#cb_context{resp_extra = RespExtra}) -> RespExtra.
...
raw_path(#cb_context{raw_path= Path}) -> Path.
raw_port(#cb_context{port= Port}) -> Port. 
raw_host(#cb_context{raw_host= Host}) -> Host.
...
scopes(#cb_context{scopes=Scopes}) -> Scopes.
...
-spec set_scopes(context(), wh_proplist()) -> context().

set_customer_id(#cb_context{}=Context, AcctId) -> Context#cb_context{customer_id=AcctId}.
set_account_id(#cb_context{}=Context, AcctId) -> Context#cb_context{account_id=AcctId}.
set_role(#cb_context{}= Context, Role) ->  Context#cb_context{role = Role}.
set_roles(#cb_context{}= Context, Roles) ->  Context#cb_context{roles = Roles}.
...
set_account_doc(#cb_context{}=Context, AcctDoc) -> Context#cb_context{db_doc=AcctDoc}.
...
set_context_data(#cb_context{}=Context, Data) -> Context#cb_context{context_data=Data}.
...
set_resp_extra(#cb_context{} = Context, RespExtra) -> Context#cb_context{resp_extra = RespExtra}.
...
set_scopes(#cb_context{}=Context, Scopes) -> Context#cb_context{scopes=Scopes}.
...
add_depreciated_validation_error(Property, Code, Message, #cb_context{validation_errors=JObj}=Context) ->
    ...
    lager:info("huet: cb_context: add validate: Api Version ~p; Message: ~p; Key: ~p; Code: ~p ~n",[ApiVersion, Error, Key, Code]),
    lager:info("huet: cb_context: JObj:  ~p ~n",[JObj]),
    ValidateErr = wh_json:set_value_props([Key, Code], Error, JObj),
    lager:info("huet: cb_context: Validate Error: ~p ~n",[ValidateErr]),
    Context#cb_context{validation_errors= wh_json:set_value_props([Key, Code], Error, JObj)
                       ,resp_status='error'
                       ,resp_error_code=400
